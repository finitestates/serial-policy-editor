...
def display(learner, runtime, chosen=1):
    ...
    io = ScriptedIO([])
    selection_notice(io, result, latent=True, token_text=runtime.backend.token_text)
...
def test_rank_one_skip_explains_unchanged_memory_and_z_bound():
    runtime = engine(latent_preference_z=(.06, .08), latent_strength=2.)
    summary, details, _ = display(LatentPreferenceLearner(FEATURES, enabled=True,
        dimension=2, latent_strength=2.), runtime)
...
def test_gate_skip_can_decay_without_learning(mode, word):
    runtime = engine(latent_preference_z=(.06, .08))
    summary, details, _ = display(LatentPreferenceLearner(FEATURES, enabled=True,
...
def test_clipped_learning_and_memory_bound_are_reported_separately():
    runtime = engine(top_k=2, latent_preference_z=(.06, .08))
    summary, details, result = display(LatentPreferenceLearner(FEATURES, enabled=True,
...
def test_zero_memory_bound_is_not_misreported_as_no_clipping():
    ...
    _, details, result = display(LatentPreferenceLearner(FEATURES, enabled=True,
...
def test_frozen_groups_are_explained_and_both_learners_share_one_report():
    ...
    latent = LatentPreferenceLearner(FEATURES, enabled=True, dimension=2).update(obs, 3, runtime.sampling)
    selection_notice(io, group, latent=False, episode_id='one')
    assert 'no eligible groups' in io.output[-1]
    selection_notice(io, latent, latent=True, episode_id='one')
    show_learning_details(io, episode_id='one')
    assert 'Groups' in io.output[-1] and 'Latent' in io.output[-1]
...
def test_write_reports_each_learners_gate_without_using_averaged_ranks():
    ...
    latent = LatentPreferenceLearner(FEATURES, enabled=True, dimension=2,
    ...
    assert 'groups 2/3 evidence' in io.output[-1] and 'latent 1/3 evidence' in io.output[-1]
    show_learning_details(io)
    details = io.output[-1]
...
def test_real_cli_callbacks_supply_text_and_keep_report_available_at_edge(tmp_path):
    ...
    backend = GateBackend()
    backend.latent_token_features = lambda **kwargs: FEATURES
    with patch('trajectory_editor.episode_cli.TerminalIO', return_value=io), \
        ...
        assert main(['--model', str(tmp_path / 'model.gguf'), '--workspace', str(tmp_path / 'run.db'),
                     '--new-prompt', 'P', '--latent-preference', '--latent-dimension', '2']) == 0
    ...
    assert sum(t.startswith('Latent [learning]') for t in io.output) == 1
...
def test_live_notice_and_details_fit_existing_surface_and_return_to_same_choice():
    ...
    result = LatentPreferenceLearner(FEATURES, enabled=True, dimension=2).update(

        ...
        while True:
            ...
            if command.kind == CommandKind.BIAS:
                ...
                if command.bias_status:
                    ...
                    preference = observation.statistics.token_preference_diagnostics
                    if preference.get("deployment_kl", 0.0) or preference.get("z_norm", 0.0):
                        lines.append(
                            f"token preference influence: mode {engine.sampling.token_preference_influence_mode}; "
                            f"target {engine.sampling.token_preference_influence_kl:.4g}; "
                            f"actual KL {preference.get('deployment_kl', 0.0):.4g}; "
                            f"gain {preference.get('effective_gain', 0.0):.4g}; "
                            f"logit RMS {preference.get('effective_logit_rms', 0.0):.4g} "
                            f"(slow {preference.get('slow_raw_logit_rms', 0.0):.4g}; "
                            f"fast {preference.get('fast_raw_logit_rms', 0.0):.4g}); "
                            f"capped {'yes' if preference.get('gain_capped', False) else 'no'}"
                        )

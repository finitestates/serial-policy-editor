...
from .output_head_vectors import (
...
def _extract(args: argparse.Namespace) -> TokenPreferenceVectorArtifact:
    if args.workspace is not None:
        ...
        with EpisodeStore(args.workspace) as store:
            ...
            episode = store.get_episode(episode_id)
            sampling = final_sampling(store, episode_id)
    ...
    try:
        ...
        if args.kind in {"output-head", "hidden-state"}:
            ...
            if args.kind == "output-head" and args.action == "derive":
                ...
                backend = _load_backend(args)
                artifact = create_output_head_prompt_pairs(
                    backend,
                    backend.provenance(include_model_sha256=True),
            ...
            if args.kind == "output-head" and args.action == "create":
                backend = _load_backend(args)
                artifact = create_output_head_prompt_pair(
                    backend,
                    backend.provenance(include_model_sha256=True),
            ...
            if args.kind == "hidden-state" and args.action == "create":
                ...
                if args.worker is not None:
                    ...
                    backend = _load_backend(args)
                    try:
                        from trajectory_editor.steering_vector_production import (
                            create_hidden_state_prompt_pair,
                        )
                    except ModuleNotFoundError as exc:
                        raise EditorError(
                            "hidden-state vector production requires the optional "
                            "policy-editor-vectors package"
                        ) from exc
            ...
            if args.kind == "hidden-state" and args.action == "import-cvector":
                ...
                if args.model is not None:
                    ...
                    artifact.validate_against_backend(
                        backend, backend.provenance(include_model_sha256=True)
                    )
                    artifact = replace(
                        artifact,

...
from .core.sampling import MAX_SEED, MIN_SEED, RNG_SCHEME
from .core.sampler_config import SAMPLING_POLICY_SCHEME, SamplerConfig
from .core.ui import ActionKind, ChoiceSet, EditAction, InsertMode
...
class SamplingConfig(SamplerConfig):
    """Compatibility record that adds research controls to ``SamplerConfig``.

    New core code should depend on ``SamplerConfig``.  This wider type remains
    responsible for loading and writing historical episode records while the
    research fields are moved behind explicit extensions.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if any(c.group not in {g.name for g in self.bias_groups} for c in controls):
    ...
    def from_record(cls, value: Mapping[str, Any]) -> "SamplingConfig":
        ...
        if "reference_prior_exit_strength" not in value:
            value["reference_prior_exit_strength"] = 0.25
        removed = {"logit_bias", "sequence_bias", "scoped_bias"} & set(value)
        if removed:
            raise EditorError(
        ...
        # Core replay records intentionally omit research-only fields. When a
        # research-facing compatibility caller opens one, treat those fields
        # as inactive defaults rather than making the core record malformed.
        for name, default in (
            ("group_control_scheme", "appearance-feedback-v1"),
            ("reference_prior_routes", ()),
            ("reference_prior_scope", "active"),
            ("reference_prior_mode", "contrastive"),
            ("reference_prior_strength", 0.25),
            ("reference_prior_attraction", 0.0),
            ("reference_prior_exit_strength", 0.25),
        ):
            value.setdefault(name, default)

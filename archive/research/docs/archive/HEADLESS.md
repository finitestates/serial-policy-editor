# SPE headless research preview

The research tree includes a local browser editor and a small HTTP API over
...
```sh
python -m trajectory_editor.research.headless --model /path/to/model.gguf
```

Open http://127.0.0.1:8765. The browser service is an optional research surface;
it is not part of the core wheel and has no `policy-editor-server` entry point.
Run it from a checkout or a research-capable installation. No additional server
or frontend packages are required. Browser assets are included in the source tree.
...
- `trajectory_editor.research.headless.Session` owns state and revision/receipt handling. The HTTP server is
...
The historical `tests/test_headless.py` exercised a real loopback HTTP server
with a deterministic backend, duplicate and stale requests, model mismatch,
sealing, and a full edit/fork/rewind/reopen journey. The headless surface is
no longer part of the active test or install surface.
